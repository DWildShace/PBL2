...
//Hàm xem và sửa điểm sinh vien trong lớp học phần
...
void removeAndAddStudentfromlhp(string LopHocPhan,string Student,LoginSystem& loginSystem){
    ...
    auto& StudentTB = loginSystem.getStudentTable();
    bool removeHayAdd=(LopHocPhanTB.contains(LopHocPhan+"-"+Student))?0:1;

    //sinh vien có tồn tại thì xoá đi
    if (removeHayAdd==0){
        for (auto& dk:DangKyTB.getAllElements()){
            if (dk.getKey()==LopHocPhan+"-"+Student){
                DangKyTB.remove(LopHocPhan+"-"+Student);
            }
        }
        cout<<"sinh vien da dc xoa";
    }else {
